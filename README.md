...
4，追加网络不良的应对机制。--ok
5，日历中的完成/未完成统计数字，改为实时计算。--ok
6，导入日历后，自动切换到刚刚导入的日历。--ok
7，编辑日历B中的日程 B1保存成功后，B1 会跑到第一个日历中。--ok
8，请求超时，将在1秒后重试 (尝试 1/3)：应该弹出提示和错误信息。--ok
9，如果因为网络问题无法上传的日程要标记未上传。--ok
10，未上传的日志不能够在同步的时候被丢弃或者覆盖。--ok
11，未上传的日志在同步时应在同步前上传，上传成功后再进行同步。--ok
...
22，任务列表增加列表吸顶。--ok
23，尝试导出 iOS 安装包上载。--ok
24，尝试导出 Android 安装包上载。
25，日程状态切换（完成/未完成）又失效（已经第4次了）。
26，添加日程失败（第 4 次）。
27，删除日程失败（第 3 次）。
28，编辑日程失败（第 3 次）。
