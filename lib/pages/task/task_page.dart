...
class _TaskPageState extends State<TaskPage> {
  ...
  Future<void> _loadTasks() async {
    ...
    try {
      ...
      final items = await _scheduleService.getSchedulesInRange(activeCalendarId, pastDate, futureDate);
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('加载任务失败: $e')));
      }
    }
  }
  ...
  void _deleteSchedule(task_models.ScheduleItem taskItem) {
    ...
    if (originalItem == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('找不到对应的日程')));
      return;
    }
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  // 搜索框
                  TextField(controller: _searchController, decoration: InputDecoration(hintText: '搜索任务...', prefixIcon: const Icon(Icons.search), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.grey[300]!)), contentPadding: const EdgeInsets.symmetric(vertical: 8), filled: true, fillColor: Colors.grey[100])),
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text(_showExpired ? "隐藏过期任务" : "显示过期任务", style: TextStyle(color: Colors.grey[700], fontSize: 14)),
                    ],
                  ),
                ],
              ),
                      ...
                      ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('没有找到匹配的任务', style: TextStyle(color: Colors.grey[600], fontSize: 16)),
                                            ...
                                            child: Row(
                                              children: [
                                                Container(
                                                  ...
                                                  child: Text('${date.month}月${date.day}日', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                                                ),
                                                const SizedBox(width: 8),
                                                Text(_getWeekday(date.weekday), style: TextStyle(color: Colors.grey[600], fontSize: 14)),
                                                const SizedBox(width: 8),
                                                if (_isToday(date)) Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: Colors.green[100], borderRadius: BorderRadius.circular(8)), child: const Text('今天', style: TextStyle(color: Colors.green, fontSize: 12, fontWeight: FontWeight.bold))),
                                                if (_isPast(date) && !_isToday(date)) Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(8)), child: const Text('已过期', style: TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.bold))),
                                              ],
                                            ),
                                          );
                                        },
                                        content: Column(
                                          children: [
                                            ...schedules
                                                ...
                                                .toList(),
                                            // 分隔线
                                            if (index < groupedSchedules.length - 1) const Divider(height: 32),
                                          ],
                                        ),
                                      ),
                                    ],
  ...
  // 构建筛选选项按钮
  Widget _buildFilterChip({required String label, required bool selected, required ValueChanged<bool> onSelected}) {
    return FilterChip(label: Text(label), selected: selected, onSelected: onSelected, checkmarkColor: Colors.white, selectedColor: Theme.of(context).colorScheme.primary, labelStyle: TextStyle(color: selected ? Colors.white : Colors.black, fontWeight: selected ? FontWeight.bold : FontWeight.normal));
  }
