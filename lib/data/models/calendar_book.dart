...
class CalendarBook {
  ...
  CalendarBook({required this.id, required this.name, required this.color, this.isShared = false, this.ownerId, this.sharedWithUsers = const [], DateTime? createdAt}) : createdAt = createdAt ?? DateTime.now();

  // 从Map创建CalendarBook对象（用于从数据库读取）
  factory CalendarBook.fromMap(Map<String, dynamic> map) {
    return CalendarBook(
      id: map['id'],
      name: map['title'], // 从数据库的title字段读取
  ...
  Map<String, dynamic> toMap() {
    return {
      'id': id,
      'title': name, // 存储到数据库的title字段
      'color': color.value,
      'is_shared': isShared ? 1 : 0,
      'owner_id': ownerId,
      'shared_with_users': sharedWithUsers,
      'created_at': createdAt.millisecondsSinceEpoch,
      'updated_at': DateTime.now().millisecondsSinceEpoch, // 添加更新时间
    };
  }
  ...
  factory CalendarBook.create({required String name, required Color color, String? ownerId}) {
    ...
    return CalendarBook(id: id, name: name, color: color, ownerId: ownerId);
  }
  ...
  CalendarBook copyWithSharedUser(String userId) {
    ...
    return CalendarBook(id: id, name: name, color: color, isShared: isShared, ownerId: ownerId, sharedWithUsers: updatedSharedUsers, createdAt: createdAt);
  }
  ...
  CalendarBook copyWithName(String newName) {
    return CalendarBook(id: id, name: newName, color: color, isShared: isShared, ownerId: ownerId, sharedWithUsers: sharedWithUsers, createdAt: createdAt);
  }

  // 更新日历本颜色
  CalendarBook copyWithColor(Color newColor) {
    return CalendarBook(id: id, name: name, color: newColor, isShared: isShared, ownerId: ownerId, sharedWithUsers: sharedWithUsers, createdAt: createdAt);
  }

  // 创建一个具有相同ID和属性的副本，但可以更改部分属性
  CalendarBook copyWith({String? name, Color? color, bool? isShared, String? ownerId, List<String>? sharedWithUsers}) {
    return CalendarBook(id: this.id, name: name ?? this.name, color: color ?? this.color, isShared: isShared ?? this.isShared, ownerId: ownerId ?? this.ownerId, sharedWithUsers: sharedWithUsers ?? this.sharedWithUsers, createdAt: this.createdAt);
  }
